...
func TestGenerateLocalMSP(t *testing.T) {
	...
	err := msp.GenerateLocalMSP(testDir, testName, nil, &ca.CA{}, &ca.TlsCA{})
	...
	// generate TLS CA
	tlsCA, err := ca.NewTlsCA(tlsCADir, testCAOrg, testCAName)
	assert.NoError(t, err, "Error generating CA")
	// generate local MSP
	...
	for _, file := range files {
		assert.Equal(t, true, checkForFile(file), "Expected to find file "+file)
	}
