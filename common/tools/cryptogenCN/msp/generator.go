...
func GenerateLocalMSP(baseDir, name string, sans []string, signCA *ca.CA,
	...
	*/
	...
	err = tlsx509Export(filepath.Join(tlsDir, "ca.crt"), tlsCA.SignCert)
	if err != nil {
		return err
	}
...
func GenerateVerifyingMSP(baseDir string, signCA *ca.CA, tlsCA *ca.TlsCA) error {
	...
	if err == nil {
		...
		// the TLS CA certificate goes into tlscacerts
		err = tlsx509Export(filepath.Join(baseDir, "tlscacerts", x509Filename(tlsCA.Name)), tlsCA.SignCert)
		if err != nil {
			return err
		}
	}
...
func tlsx509Export(path string, cert *x509.Certificate) error {
	return pemExport(path, "CERTIFICATE", cert.Raw)
}
