...
func TestGeneratePrivateKey(t *testing.T) {
	...
	//The key is stored in HSM
	/*
		pkFile := filepath.Join(testDir, hex.EncodeToString(priv.SKI())+"_sk")
		t.Log(pkFile)
		assert.Equal(t, true, checkForFile(pkFile), "Expected to find private key file")
		cleanup(testDir)
	*/

}

//GeneratePrivateKey return a *sm2.PrivateKey
